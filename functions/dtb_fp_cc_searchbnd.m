...
% norm to 1
prior = prior(:)'/sum(prior); % column vec of prior over coherences
defective_dt_dist = 1-cumsum(dtdist/nansum(dtdist)); % probability of decision time (Td) terminated at time t
...
for k = 2:nt
    ...
    % Propagation: 
    % In this step, we propagate the probability distribution of 
    % the decision variable for a single time step.
    u = M\u; % 'Propagation'
    
    ur = reshape(u,ny,nd);
    
    % 'Match': 
    % Since there is a monotonic relation between the bound height and 
    % the proportion of DV that crossed the bound, we can set the height of 
    % the symmetric bounds ±B(t) to match the probability of decision time (Td) 
    % terminated at time t and the probability of DV that exceeds the two bounds 
    % at time t
    % search B(k)
