...
% half the variance of the momentary evidence; not the variance as indicated in kiani2009
D = 0.5 * var;
...
for idrift = 1:nd
    a = -1*drift(idrift);
    w = dy*a/D(idrift);
    %if drift is close to zero consider it zero to avoid problems with
    %singular matrix
    if abs(a)<1e-9
        delta = 0.5;
    else
